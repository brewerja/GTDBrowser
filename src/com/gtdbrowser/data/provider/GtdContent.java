...
public abstract class GtdContent {
	...
	public static final class AttackDao extends GtdContent implements AttackDaoColumns, BaseColumns {
		...
		public static final int CONTENT_YEAR_COLUMN = 2;
		public static final int CONTENT_MONTH_COLUMN = 3;
		public static final int CONTENT_DAY_COLUMN = 4;
		public static final int CONTENT_DATE_COLUMN = 5;
		public static final int CONTENT_APPROXDATE_COLUMN = 6;
		public static final int CONTENT_EXTENDED_COLUMN = 7;
		public static final int CONTENT_RESOLUTION_COLUMN = 8;
		public static final int CONTENT_COUNTRY_COLUMN = 9;
		public static final int CONTENT_REGION_COLUMN = 10;
		public static final int CONTENT_PROVSTATE_COLUMN = 11;
		...
		static void createTable(final SQLiteDatabase db) {
			final String s = " (" + _ID + " integer primary key autoincrement, " + ID + " text, " + YEAR + " integer, "
					...
					+ " double, " + RANSOMPAIDUS + " double, " + RANSOMNOTE + " text, " + HOSTKIDOUTCOME + " text, "
					+ NRELEASED + " double, " + ADDNOTES + " text, " + SCITE1 + " text, " + SCITE2 + " text, " + SCITE3
					+ " text, " + DBSOURCE + " text " + ");";
		...
		public static String getBulkInsertString() {
			...
			sqlRequest.append(", ");
			sqlRequest.append(YEAR);
			sqlRequest.append(", ");
			sqlRequest.append(MONTH);
			sqlRequest.append(", ");
			sqlRequest.append(DAY);
			sqlRequest.append(", ");
		...
		public static void bindValuesInBulkInsert(final SQLiteStatement stmt, final ContentValues values) {
			...
			Integer valInt = values.getAsInteger(YEAR);
			if (valInt != null)
				stmt.bindLong(i++, valInt);
			else
				stmt.bindNull(i++);
			valInt = values.getAsInteger(MONTH);
			if (valInt != null)
				stmt.bindLong(i++, valInt);
			else
				stmt.bindNull(i++);
			valInt = values.getAsInteger(DAY);
			if (valInt != null)
				stmt.bindLong(i++, valInt);
			else
				stmt.bindNull(i++);
			...
			stmt.bindString(i++, value != null ? value : "");
			valInt = values.getAsInteger(NPERPS);
			if (valInt != null)
				stmt.bindLong(i++, valInt);
			else
				stmt.bindNull(i++);
